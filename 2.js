const PUZZLE_INPUT = `ayitmcjvlhedbsyoqfzukjpxwt
...
const findAllAppearancesOfOneLetterInGivenPhrase = (arr, letter) => {
  ...
  arr.forEach(item => item.includes(letter) ? numberOfAppearances++ : numberOfAppearances);
  return numberOfAppearances;
};

const getNumberOfAppearancesForEachLetterInGivenPhrase = (arr) => {
  const appearances = {};
  const uniqueArr = new Set(arr);
  for (const letter of uniqueArr) {
    appearances[letter] = findAllAppearancesOfOneLetterInGivenPhrase(arr, letter);
  }
  return appearances;
};

const getNumberOfPhrasesWithLetterAppearingNTimes = (letterAppearances, n) => {
  return Object.values(letterAppearances).filter(item => Object.values(item).includes(n)).length;
}

const getFinalChecksum = (arrOfIds) => {
  const letterAppearances = {};
  for (const id of arrOfIds) {
    letterAppearances[id] = getNumberOfAppearancesForEachLetterInGivenPhrase(id);
  }

  const numberOfTwoAppearances = getNumberOfPhrasesWithLetterAppearingNTimes(letterAppearances, 2);
  const numberOfThreeAppearances = getNumberOfPhrasesWithLetterAppearingNTimes(letterAppearances, 3);

  return numberOfTwoAppearances * numberOfThreeAppearances;
};
