...
const getFirstDuplicate = (arr) => {
  const dictionary = {};

  for (const [i, val] of arr.entries()) {
    if (dictionary[val] !== undefined) {
      return arr[dictionary[val]];
    }
...
const reduceFrequencies = (arr, sums) => arr.reduce((acc, val) => {
  const sum = acc + val;
  sums.push(sum);
  return sum;
});

const getFirstFrequency = (arr) => {
  const sums = [];
  ...
  for (const num of arrCopy) {
    if (sums.length) {
      ...
      if (hasDuplicates(sums)) {
        return getFirstDuplicate(sums);
      }

      if (arrCopy.length === arr.length) {
        arrCopy.unshift(lastSum);
      } else {
        arrCopy[0] = lastSum;
      }
    }

    reduceFrequencies(arrCopy, sums);
  }
};
